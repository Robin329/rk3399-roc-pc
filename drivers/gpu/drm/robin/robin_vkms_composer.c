				 ...
				 struct robin_vkms_crtc_state *crtc_state)
{
	...
	pr_info("vaddr:%p primary_composer.offset:%d pitch:%d cpp:%d\n", vaddr, primary_composer->offset, primary_composer->pitch, primary_composer->cpp);
...
void robin_vkms_composer_worker(struct work_struct *work)
{
	...
	pr_info("crc32:%d vaddr_out:0x%p\n", crc32, vaddr_out);
	if (wb_pending) {

...
int spi_write_slt(int id, const void *txbuf, size_t n) {
    ...
    if (id >= MAX_SPI_DEV_NUM) return ret;
...
static ssize_t spi_test_write(struct file *file, const char __user *buf, size_t n, loff_t *offset) {
    ...
    if (copy_from_user(tmp, buf, n)) return -EFAULT;
    cmd = tmp;
    data = tmp;
    ...
    if (!strcmp(cmd, "setspeed")) {
        ...
        if (id >= MAX_SPI_DEV_NUM) return n;
        if (!g_spi_test_data[id]) {
            pr_err("g_spi.%d is NULL\n", id);
            return n;
        } else {
        ...
        for (i = 0; i < times; i++) spi_write_slt(id, txbuf, size);
        end_time = ktime_get();
        ...
        start_time = ktime_get();
        for (i = 0; i < times; i++) spi_read_slt(id, rxbuf, size);
        end_time = ktime_get();
        cost_time = ktime_sub(end_time, start_time);
        us = ktime_to_us(cost_time);
        ...
        print_hex_dump(KERN_ERR, "SPI RX: ", DUMP_PREFIX_OFFSET, 16, 1, rxbuf, size, 1);
...
static int rockchip_spi_test_probe(struct spi_device *spi) {
    ...
    if (!spi->dev.of_node) return -ENOMEM;
